...
def test_payments_new_queue_configuration():
    ...
    assert payments_new_queue.arguments["x-dead-letter-exchange"] == PAYMENTS_DLX_EXCHANGE_NAME
    assert payments_new_queue.arguments["x-dead-letter-routing-key"] == PAYMENT_FAILED_ROUTING_KEY


def test_retry_queues_use_exponential_backoff_and_return_to_main_exchange():
